...
| CI | GitHub Actions يشغّل التحقق واختبارات الوحدة والمتصفح على كل push |
| الأمان | CSP في `index.html` + تهريب شامل + فلترة روابط |
...
│
├── files/                           # ★ الملفات الفعلية (PDF/Word/PPT/Excel/صور)
│   └── <مجلد لكل مادة>              # التفاصيل في files/README.md
│
├── .github/workflows/ci.yml         # فحص آلي على كل push
├── robots.txt · sitemap.xml         # الفهرسة
│
├── scripts/
...
```bash
npm test                  # 44 اختبار وحدة (بيانات، بحث، تصحيح، توجيه، أمان، ترجمة)
npm run test:e2e          # 58 اختبار متصفح (وظيفي + استجابة + وصول + حفظ التقدّم + console)
...
**أين تُرفع الملفات الفعلية؟** في مجلد `files/` — مجلد جاهز لكل مادة:

```
files/
├── ai-data/                         # الذكاء الاصطناعي وتحليل البيانات
├── legal-regulatory/                # الثقافة القانونية والتنظيمية
├── cybersecurity-governance/        # حوكمة الأمن السيبراني
├── innovation-project-management/   # الابتكار وإدارة المشاريع
└── risk-management/                 # إدارة المخاطر
```

ارفع الملف داخل مجلد مادته، ثم ضع مساره النسبي في `url`:
`url: 'files/ai-data/lecture-01.pdf'` — بلا `/` في أوله وبلا رابط GitHub كامل.

استخدم أسماء إنجليزية بلا مسافات (`lecture-01.pdf`). للملفات الكبيرة (فيديو) ارفعها
على YouTube أو Drive وضع الرابط الكامل في `url` بدل المسار.
تفاصيل كاملة في `files/README.md`.
...
* **الإنجليزية:** جدول `en` **جاهز ومكتمل** (128 مفتاحاً مطابقاً للعربية) في
  `data/config/strings.js`. لتفعيله نفّذ `DLP.i18n.setLocale('en')` — يبدّل `lang` و`dir`
  تلقائياً. ملاحظة: يترجم الواجهة فقط؛ المحتوى الدراسي يبقى بلغته الأصلية.
  اختبار آلي يمنع أي نص عربي جديد من التسرّب إلى طبقة الواجهة.
...
## 7-ب. سلوكيات إضافية مهمة

**حفظ تقدّم الاختبار:** يُحفظ تلقائياً في `localStorage` لكل اختبار على حدة
(`dlp.quiz.<معرّف الاختبار>`)، ويُستعاد عند العودة للصفحة. البيانات محلية على جهاز الدارس
ولا تُرسل إلى أي خادم. زر «مسح التقدّم المحفوظ» يمسحها، والمنصة تعمل طبيعياً إذا كان
التخزين معطّلاً (وضع التصفح الخاص) — مُختبَر آلياً.

**إمكانية الوصول:** عند التنقل بين الصفحات يُنقل التركيز إلى عنوان الصفحة ويُعلَن التغيير
عبر منطقة `aria-live`. التبويبات تطبّق `roving tabindex` مع دعم الأسهم وHome وEnd وفق
WAI-ARIA APG. عند أول تحميل لا يُنقل التركيز حفاظاً على ترتيب Tab الطبيعي (رابط التخطي أولاً).

**سياسة الملفات الكبيرة:** حد GitHub 100MB للملف الواحد، والملفات المرفوعة تبقى في تاريخ
Git للأبد. للملفات فوق ~20MB (فيديو، مسح ضوئي عالي الدقة) **ارفعها على استضافة خارجية**
(YouTube/Drive) وضع الرابط في `url` بدل المسار — يبقي المستودع خفيفاً وسريع الاستنساخ.
...
* **CSP** في `index.html`: `script-src 'self'` (لا سكربتات خارجية إطلاقاً)، والخطوط من
  Google Fonts حصراً، و`object-src 'none'` و`base-uri 'none'` و`form-action 'none'`.

---

## 9. أرشيف الكورس الأول

`files/archive/course1-question-bank.html` — بنك أسئلة الكورس الأول
(الإدارة المالية الرقمية، 360 سؤالاً) كصفحة مستقلة، **مرتبط من صفحة «من نحن» ← قسم
الأرشيف**. محتواه لم يُعدّل. لإضافة أرشيف آخر: أضف عنصراً إلى مصفوفة `archive` في
`data/config/about.js`.

> كان الملف في الجذر باسم عربي يحوي محارف اتجاه غير مرئية (`U+200E`/`U+2068`/`U+2069`)،
> فكان يعمل محلياً ويفشل على بيئات أخرى. نُقل إلى اسم آمن، ويمنع اختبار آلي الآن أي اسم
> ملف يحوي محارف غير مرئية — وهذا سبب إضافي للالتزام بأسماء إنجليزية بلا مسافات.
