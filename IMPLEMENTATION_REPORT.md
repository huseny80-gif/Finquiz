...
**الحالة العامة: غير مكتمل — المراحل 0 (القاعدة/الأمان)، 1 (طبقة الاتصال + `hydrate()`)، 2
(حفظ دائم لنتائج الاختبار، جزئياً)، و3 (لوحة الطالب، بتبسيط مقصود — انظر §"المرحلة 3" أدناه)
منجزة ومُختبرة، لكن **القراءة الحية من القاعدة معطَّلة عمداً** (`enabled: false`) بسبب فجوة
`question_pairs` غير المحلولة. الموقع اليوم يعمل حصراً من الملفات الثابتة تماماً كما كان قبل أي
من هذا العمل — لا فرق سلوكي ملحوظ لأي زائر (لوحة الطالب تظهر بطاقة "قريباً" للجميع). سقالة
الإدارة (المرحلة 4) لم تبدأ.**
...
- `reveal_question_answer` (المرحلة 1) لا يزال غير مُستخدَم من أي مكوّن — الشرح المعروض اليوم
  ...
  الشرح أصلاً).
- لا سقالة `components/admin/` (المرحلة 4).
- لا اختبار واحد نُفِّذ بعد ضد مشروع Supabase حقيقي متصل عبر متصفح فعلي (مثل تسجيل دخول Google
  حقيقي، وحفظ محاولة فعلية عبر الشبكة، وعرضها في لوحة الطالب من بيانات حقيقية) — كل ما تحقّق حتى
  الآن استخدم عملاء/بيانات وهمية محلياً في Node، أو استعلامات SQL مباشرة (بلا RPC/auth.uid()
  حقيقي). هذا فرق جوهري يجب سدّه قبل أي تفعيل حي لتسجيل الدخول.
